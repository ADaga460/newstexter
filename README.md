...
- **Curation + replies** use Claude (`claude-opus-4-8`); replies use the web-search tool.
...
1. **Scheduled digest** (APScheduler): fetch RSS → drop stale/already-sent →
   Claude selects, tiers, and writes each story → text via Twilio.
2. **Inbound replies** (`POST /sms`): a Twilio webhook routes texts from
   allowlisted numbers to Claude (+ web search), with short per-number history.
